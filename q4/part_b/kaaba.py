from world_3 import World, normalise_vector, vector_length, get_nearest_position
import math
import numpy as np
...
def update_directions(pedestrians, boundaries, polygons, centre=np.array([0, 0])):
    for i, ped in pedestrians.items():
        ...
        if not ped.has_started:
            ...
            if ped.num_circles < 4:
                ped.desired_direction = combined_direction
                angle_to_centre = math.atan2(ped.pos[1] - centre[1], ped.pos[0] - centre[0])
                if (ped.prev_angle <= angle_to_centre and ped.prev_angle <= ped.start_angle <= angle_to_centre) or (ped.prev_angle > angle_to_centre and (ped.start_angle >= ped.prev_angle or ped.start_angle <= angle_to_centre)):
                    ped.num_circles += 1
                    #print("circle count increased by 1")
                ped.prev_angle = angle_to_centre  # Update previous angle for next iteration. maybe need to record if it has passed over the angle instead

            else:
                # After completing 7 circles, the pedestrian starts moving away tangentially from the center
                # Calculate direction tangentially away from the center
                direction_away_from_center = normalise_vector(ped.pos - centre)
                # Set the direction to be tangential away from the center
                ped.desired_direction = direction_away_from_center

def process_interactions(pedestrians, boundaries, polygons):
    # Parameters
    tau = 0.5
    A_boundary_social, B_boundary_social = 1.5, 3
    A_boundary_physical = 60
    A_social, B_social = 1, 2
    A_physical = 4
    Lambda = 0.2
...
world_definition = {
    "space": {
        #"walking space": {"type": "polygon", "coordinates": [[-40, -40], [-40, -38], [38, -38], [38, 38], [-38, 38], [-38,-38], [-40,-38], [-40, 40], [40, 40], [40, -40]], "colour": "white", "add_boundaries": False},
        #"walking space": {"type": "polygon", "coordinates": [[-30, -30], [-30, -28], [28, -28], [28, 28], [-28, 28], [-28,-28], [-30,-28], [-30, 30], [30, 30], [30, -30]], "colour": "red", "add_boundaries": False},
        "walking space": {"type": "rectangle", "coordinates": [-30, 29, 30, 30], "colour": "white", "add_boundaries": False},
        #"walking space": {"type": "rectangle", "coordinates": [-40, -40, 40, 40], "colour": "red", "add_boundaries": False},
    ...
    "pedestrians": {
        "group1": {"source": "walking space", "destination": "end", "middle": "kaaba", "colour": "red", "birth_rate": 0.5, "max_count": 100},
    },
...
world.save_animation('kaaba_please2.gif')

# improvements - figure out the polygon where people get generated
