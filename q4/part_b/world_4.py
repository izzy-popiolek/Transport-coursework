...
class Pedestrian:
    def __init__(self, id, birth_time, starting_position, source, destination, middle, radius, colour, group_name=None):
        self.id = id
        ...
        self.desired_walking_speed = random.gauss(1.34, 0.37)# random.gauss(1.34, 0.37)
        self.metadata = {}
        ...
        self.prev_angle = 0
        self.rand = random.randint(1, 150)
...
class World:
    ...
    def update_positions(self, dt=0.02): # was 0.05
    ...
    def birth_pedestrians(self, dt):
        for group_name, data in self.pedestrian_sources.items():
            ...
            while len(self.get_statistics(["source", group_name]))<max_count and random.random() < dt*birth_rate:
                radius = 0.4 + 0.1*random.random() # i have changed this first number from 0.4
                x0, y0 = self.get_random_position(polygon_name=data["source"], margin=radius)
                ped = Pedestrian(self.next_pedestrian_id, self.time, np.array([x0, y0]), data["source"], data["destination"], data["middle"], radius, data["colour"], group_name=group_name)
    ...
    def remove_pedestrians(self):
        ...
        for i, ped_i in self.pedestrians.items():
            ...
            if self.is_inside_polygon(ped_i.pos, ped_i.middle):
                #self.add_to_statistics(["completed_journeys", ped_i.source, ped_i.destination], self.time - ped_i.birth_time)
                keys_to_remove.append(i)
